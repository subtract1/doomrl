	...
	OnTick = function ()
		if(level.status <= 500) then
			if (player.position.x > 58 or level.status == 500) then
				...
				--Spawn Hans.  Since Hans also acts as a 'regular' enemy we must make 'boss level' upgrades to his stats.
				--We cannot change the AI (not reasonably anyway) or the descriptive text (that must remain neutral).
				local boss = level:drop_being("wolf_bosshans", coord.new(68,11))
				boss.hpmax = (boss.hpmax + 20) + DIFFICULTY * DIFFICULTY * 2.5
				boss.hp = boss.hpmax
				boss.flags[BF_HUNTING] = true
				boss.expvalue = 0
				boss.tohitmelee = boss.tohitmelee + 2
